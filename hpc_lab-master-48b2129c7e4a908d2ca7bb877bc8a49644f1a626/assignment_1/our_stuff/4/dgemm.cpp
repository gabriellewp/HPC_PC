...
void dgemm_opt(double* A, double* B, double* C) {
	...
	for(int i = 0; i < M*N; i+=8){
		C_reg[i/8] = _mm512_load_pd(&C[i]);
	}

	for (int n = 0; n < N; ++n) {
		for (int k = 0; k < K; ++k) {
			A_reg = _mm512_load_pd(&A[k*M]);
			B_reg = _mm512_set1_pd(B[n*K + k]);
			C_reg[n] = _mm512_fmadd_pd(A_reg, B_reg, C_reg[n]);
		}
	}

	for(int i = 0; i < M*N; i+=8){
		_mm512_store_pd(&C[i], C_reg[i/8]);
	}
